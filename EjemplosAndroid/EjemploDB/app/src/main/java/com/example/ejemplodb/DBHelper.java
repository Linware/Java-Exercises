...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String sql = String.format("create table %s (%s integer primary key, " +
                "%s text, %s text, %s text, %s text, %s text)", TABLE_PLACES, COL_ID, COL_NAME, COL_CITY, COL_URL,COL_LON,COL_LAT);
        db.execSQL(sql);

        //Insertamos un par de registros para tener algo que listar (carga inicial):
        sql = String.format("insert into %s (%s, %s, %s, %s, %s) values ('Museo Guggenheim','Bilbao','http://4learning.bymhost.com/Android/fotos/gug.jpg','43.269209', '-2.934119')",
                TABLE_PLACES, COL_NAME, COL_CITY, COL_URL,COL_LON,COL_LAT);
        db.execSQL(sql);
        sql = String.format("insert into %s (%s, %s, %s, %s, %s) values ('Torre Eiffel','Paris','http://4learning.bymhost.com/Android/fotos/eif.jpg','48.858589', '2.294503')",
                TABLE_PLACES, COL_NAME, COL_CITY, COL_URL,COL_LON,COL_LAT);
        db.execSQL(sql);
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        if (newVersion>oldVersion) {
            ...
            sql = String.format("update %s set %s='http://4learning.bymhost.com/Android/fotos/gug.jpg' where %s='1'", TABLE_PLACES, COL_URL, COL_ID);
            db.execSQL(sql);
            sql = String.format("update %s set %s='http://4learning.bymhost.com/Android/fotos/eif.jpg' where %s='2'", TABLE_PLACES, COL_URL, COL_ID);
            db.execSQL(sql);
        }
        ...
        if (res.moveToFirst()) {
            do {
                JPlace p = new JPlace(res.getInt(res.getColumnIndex(COL_ID)),
                        ...
                        res.getString(res.getColumnIndex(COL_LON)),
                        res.getString(res.getColumnIndex(COL_LAT)));
                list.add(p);
